...
BEGIN_MESSAGE_MAP(CMainFrame, CFrameWndEx)
	...
	ON_COMMAND(ID_MACHINE_MACHINEASETNUMBER, &CMainFrame::OnMachineMachineasetnumber)
	ON_COMMAND(ID_MACHINE_MACHINEBSETNUMBER, &CMainFrame::OnMachineMachinebsetnumber)
	ON_COMMAND(ID_MACHINEA_PROPERTIES, &CMainFrame::OnMachineaProperties)
	ON_COMMAND(ID_MACHINEB_PROPERTIES, &CMainFrame::OnMachinebProperties)
END_MESSAGE_MAP()
...
void CMainFrame::Initialize()
{
	...
	mPicture->SetMainFrame(this);
	//Get assoications to the machines
	mActorMachineA = factory.GetActorMachineA();
	mActorMachineB = factory.GetActorMachineB();
	mDrawableMachineA = factory.GetDrawableMachineA();
	mDrawableMachineB = factory.GetDrawableMachineB();
...
void CMainFrame::OnEditTimelineproperties()
{
    ...
    if (dlg.DoModal() == IDOK)
    {
        dlg.Take();
        mPicture->UpdateObservers();
		//Machines frame rate also needs to get updated. 
		mDrawableMachineA->SetFrameRate(mPicture->GetTimeline()->GetFrameRate());
		mDrawableMachineB->SetFrameRate(mPicture->GetTimeline()->GetFrameRate());
    }
}

/**
 * WHen changing machine number of machine A
 */
void CMainFrame::OnMachineMachineasetnumber()
{
	mDrawableMachineA->ShowMachineSeedDialogBox();
	mPicture->UpdateObservers();
}


/**
 * WHen changing machine number of machine B
 */
void CMainFrame::OnMachineMachinebsetnumber()
{
	mDrawableMachineB->ShowMachineSeedDialogBox();
	mPicture->UpdateObservers();
}


/**
 * When changing start frame and position for machine a
 */
void CMainFrame::OnMachineaProperties()
{
	CMachineDrawableDlg dlg;
	dlg.SetMachineDrawable(mDrawableMachineA);
	dlg.SetActor(mActorMachineA);
	if (dlg.DoModal() == IDOK)
	{
		dlg.Take();
		mPicture->UpdateObservers();
...
/**
 * When changing properties for machine B
 */
void CMainFrame::OnMachinebProperties()
{
	CMachineDrawableDlg dlg;
	dlg.SetMachineDrawable(mDrawableMachineB);
	dlg.SetActor(mActorMachineB);
	if (dlg.DoModal() == IDOK)
	{
		dlg.Take();
		mPicture->UpdateObservers();
