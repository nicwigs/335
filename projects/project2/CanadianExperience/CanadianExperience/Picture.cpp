...
void CPicture::Save(const std::wstring &filename)
{
	...
	//Save specific machine info.
	mMainFrame->GetDrawableMachineA()->Save(root,L"A");
	mMainFrame->GetDrawableMachineB()->Save(root,L"B");
...
void CPicture::Load(const std::wstring &filename)
{
    ...
    try
    {
		...
		//Load specific machine info.
		mMainFrame->GetDrawableMachineA()->Load(root,L"A");
		mMainFrame->GetDrawableMachineB()->Load(root,L"B");
