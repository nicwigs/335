...
BEGIN_MESSAGE_MAP(CChildView, CWnd)
	...
	ON_COMMAND(ID_MINION_MINION, &CChildView::OnMinionMinion)
	ON_COMMAND(ID_MINION_BANANA, &CChildView::OnMinionBanana)
	ON_UPDATE_COMMAND_UI(ID_MINION_BANANA, &CChildView::OnUpdateMinionBanana)
	ON_COMMAND(ID_MINION_POTION, &CChildView::OnMinionPotion)
	ON_UPDATE_COMMAND_UI(ID_MINION_POTION, &CChildView::OnUpdateMinionPotion)
	ON_COMMAND(ID_MINION_RUNAWAY, &CChildView::OnMinionRunaway)
	ON_UPDATE_COMMAND_UI(ID_MINION_RUNAWAY, &CChildView::OnUpdateMinionRunaway)
END_MESSAGE_MAP()
/// \endcond
...
void CChildView::OnLButtonDown(UINT nFlags, CPoint point)
{
    ...
    {
		if (mGiveMinionPotionActive)
		{
			// Instantiate the visitor
			CGiveMinionsPotionVisitor visitor;
		...
		else if (mFeedMinionsActive)
		{
			// Instantiate the visitor
			CFeedMinionsVisitor visitor;
...
/**
 * Menu Handler that shows mouse movement
 */
...
/**
 * Menu handler that updates checkmark on mouse track option
 * \param pCmdUI to check menu box
 */
...
/**
*  Add a CTileMinion tile to the drawing.
*/
void CChildView::OnMinionMinion()
{
	auto tile = make_shared<CTileMinion>(&mCity);
	tile->SetLocation(InitialX, InitialY);
	mCity.Add(tile);
	Invalidate();
}
...
/**
 * Handle the Minion>feed menu option
 */
void CChildView::OnMinionBanana()
{
	mFeedMinionsActive = !mFeedMinionsActive;
	if (mFeedMinionsActive) //dont allow both buttons on at once
	{
		mGiveMinionPotionActive = false;
	}
}

/**
* Update the checkmark for the Minion>Banana menu option
*
* \param pCmdUI allows change of check and enablablity
*/
void CChildView::OnUpdateMinionBanana(CCmdUI *pCmdUI)
{
	CMinionCounter visitor;
	mCity.Accept(&visitor);
	int minionCnt = visitor.GetNumMinions();

	if (minionCnt > 0)
	{
		pCmdUI->Enable(true);  		// Will enable the menu option
		pCmdUI->SetCheck(mFeedMinionsActive);
	}
	else
	{
...
/**
* Handle the Minion>potion menu option
*/
void CChildView::OnMinionPotion()
{
	mGiveMinionPotionActive = !mGiveMinionPotionActive;
	if (mGiveMinionPotionActive)  //only allow feed or potion not both to be active
	{
		mFeedMinionsActive = false;
	}
}


/**
 * Update potion option depending on number of mutants
 * 
 * \param pCmdUI allows change of check and enablablity
 */
void CChildView::OnUpdateMinionPotion(CCmdUI *pCmdUI)
{
	CMinionCounter visitor;
	mCity.Accept(&visitor);
	int mutantCnt = visitor.GetNumMutants();

	if (mutantCnt > 0)
	{
		pCmdUI->Enable(true);  		// Will enable the menu option
		pCmdUI->SetCheck(mGiveMinionPotionActive);
	}
	else
	{
		pCmdUI->Enable(false);     // Will disable the menu option
...
/**
* Handle the Minion>Run Away menu option
*/
void CChildView::OnMinionRunaway()
{
	CMinionRunAwayVisitor visitor;
	mCity.Accept(&visitor);
}


/**
 * Update if a minion can run away or not
 *
 * \param pCmdUI controls enable menu option
 */
void CChildView::OnUpdateMinionRunaway(CCmdUI *pCmdUI)
{
	CMinionCounter visitor;
	mCity.Accept(&visitor);
	int minionCnt = visitor.GetNumMinions();
