...
namespace Testing
{
	...
	{
	public:
		virtual void VisitRoad(CTileRoad *road) override { mNumRoads++; mVisitedRoads = true; }
		int mNumRoads = 0;
		bool mVisitedRoads = false;

		virtual void VisitLandscape(CTileLandscape *landscape) override
			{ mNumLandscapes++; mVisitedRoads = true;}
		int mNumLandscapes = 0;
		bool mVisitedLandscapes = false;


		virtual void VisitCoalmine(CTileCoalmine *coalmine) override 
			{ mNumCoalmines++; mVisitedCoalmines = true;}
		int mNumCoalmines = 0;
		bool mVisitedCoalmines = false;


		virtual void VisitBuilding(CTileBuilding *building) override
			{ mNumBuildings++; mVisitedBuildings = true;}
		int mNumBuildings = 0;
		bool mVisitedBuildings = false;
	...
	public:
		...
		TEST_METHOD(TestCCityVisitor)
		{
			...
			Assert::IsFalse(emptyVisitor.mVisitedRoads, L"Didnt visit roads");
			Assert::IsFalse(emptyVisitor.mVisitedLandscapes, L"Didnt visit landscapes");
			Assert::IsFalse(emptyVisitor.mVisitedCoalmines, L"Didnt visit coalmines");
			Assert::IsFalse(emptyVisitor.mVisitedBuildings, L"Didnt visit buildings");
