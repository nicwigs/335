...
void CActor::SetPicture(CPicture *picture)
{
	...
	//Set channel name for position channel
	mChannel.SetName(mName);

	//Set the timeline for the actor. Links channel to timeline
...
{
	//Set keyframe for position channel
...
{
	//Get current actor channel from animation system
	if (mChannel.IsValid())
...
/**
* Deletes keyframes at current frame.
*/
void CActor::DeleteKeyframe()
{
	//Delete keyframe for position channel
	mChannel.DeleteKeyframe();

	//delete keyframe for all drawables in actor.
	for (auto drawable : mDrawablesInOrder)
	{
