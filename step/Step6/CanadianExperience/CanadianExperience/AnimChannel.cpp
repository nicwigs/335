...
{
	//if keyframe1 is -1 then there are no keyframes. 
	if (mKeyframe1 >= 0)
	{
		//Get current frame
		int currFrame = mTimeline->GetCurrentFrame();

		//get frame at keyframe1
		int frame1 = mKeyframes[mKeyframe1]->GetFrame();

		//Did we click where a frame is
		if (currFrame == frame1) 
		{
			mKeyframes.erase(mKeyframes.begin() + mKeyframe1); //delete
			mKeyframe1--; //already know this is at least 0, can subtract.
			if (mKeyframe2 >= 0)
			{
				mKeyframe2--; //subtract 1 unless this was last/only keyframe
			}
		}
	}
}
