...
/// The time to open the door once elevator stoped moving
const double DoorTimeTillOpen = 1.0;
...
void CController::OnOpenPressed()
{
	...
	case DoorClosing: //Press open when door closes
		SetState(DoorOpening);
		break;
...
void CController::OnClosePressed()
{
	...
	case DoorOpening: //Close pressed when doors opening
		SetState(DoorClosing);
		break;

	case DoorClosing: //entry activity for door closing
...
void CController::Service()
{
	...
	case Stop: //stop should open door TASK
	{
		int floor = WhatFloorToGoTo();
		if (floor == mFloor && (mStateTime >= DoorTimeTillOpen))
		{
			// We just reached this floor, open the door!
			SetState(DoorOpening);
		}
	}
	break;
